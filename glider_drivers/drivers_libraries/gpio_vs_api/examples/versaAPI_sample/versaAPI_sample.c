...
void runDIOSimpleExample(int channel, char *channelName)
{
	...
	(void)VSL_DIOSetupInterrupts(channel, curPid);
	/* ********************************************************* */
    ...
    while(gLoopCount <= gMaxNumLoops)
    {
		...
		printf("Setting channel direction\n");
		VSL_DIOSetChannelDirection(channel, DIO_INPUT);

	    // Report on the DIO level.
        dioLevel = VL_DIOGetChannelLevel(channel);
		printf("The LEVEL of %s (0x%x) is: %x\n", channelName, channel, dioLevel);
				           ...
				           int inputChannel,  char *inputChannelName)
{
	...
	curPid = getpid();
	printf("Registering DIO/GPIO interrupt handler for process %ld.\n", (long)curPid);
	(void)VSL_DIOSetupInterrupts(inputChannel, curPid);
	/* ********************************************************* */
...
int main(int argc, char *argv[])
{
    ...
    currentDIO 	   = DIO_CHANNEL_8;
	snprintf(currentDIOName, sizeof("On-board DIO_CHANNEL_6"), "On-board DIO_CHANNEL_6");
